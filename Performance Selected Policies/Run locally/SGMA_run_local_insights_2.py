...
#######################################################################################
# Import data
SEMI_WEST_df = pd.read_csv("Westlands_Semitropic_data.csv") 
...
# index1 = results_Borg.iloc[:,-5].idxmin() #solution that optimizes net revenues (max)       
# # index2 = results_Borg.iloc[:,-4].idxmin() 

# dy = results_Borg.iloc[:,-1]  
# index2 = np.abs(dy - (-0.8)).argmin() 
# index3 = np.abs(dy - (-0.5)).argmin()
...
# Robust final
index1 = 62 #robust max rev
# index2 = 14
# index3 = 80 #robut min depth
...
swdi = np.argmax(SOW["surface_water"].std(axis=1)) # high sd
...
s = 50 # Same samples for other vairables other than surface water supplies and price of surface water
...
for i in range(4):       
    ...
    vars = results_Borg.iloc[list_index[i],:].tolist()
    vars = vars[0:40]
    ...
    for y in range(nYears):
        ...
        if y == 0:
           ...
           scenario = scenario_run(year = y, sw_supply = SOW["surface_water"].loc[swdi,y], gwrest = pump_capacity, peren_crops = Perennials_t_1,
                                   pump_tax=0,prices=SOW["Prices"].loc[s,y], sw_price=SOW["swprice"].loc[swdi,y], gwdepth=GWD_t_1,
                                   costs=costs, land_avail=land_max, elec_price=SOW["Omegaelec"].loc[s,y], yields=np.array([1]*17),peren_restrict=land_max,
                                   executable=executable_path, Land_t_1 = None) # This creates a dictionary that is the input for the model run
           ...
           depth = depth_change_func(district=district,gwmodel=gwmodel,wtr_yr=SOW["wateryear"].loc[swdi,y],wtr_yr_lag=WY_t_1 ,pumping=Pump_end_t[y],
           ...
           scenario = scenario_run(year = y, sw_supply = SOW["surface_water"].loc[swdi,y], gwrest = pump_capacity, peren_crops = Perennials_end_t[y-1],
                                   pump_tax=0,prices=SOW["Prices"].loc[s,y], sw_price=SOW["swprice"].loc[swdi,y], gwdepth=GW_depth_end_t[y-1],
                                   costs=costs, land_avail=land_max, elec_price=SOW["Omegaelec"].loc[s,y], yields=np.array([1]*17),peren_restrict=Perennials_end_t[y-1]*1.05,
                                   executable=executable_path, Land_t_1 = None) # This creates a dictionary that is the input for the model run
           ...
           # Groundwater depth forecast
           depth = depth_change_func(district=district,gwmodel=gwmodel,wtr_yr=SOW["wateryear"].loc[swdi,y],wtr_yr_lag=SOW["wateryear"].loc[swdi,y-1],pumping=Pump_end_t[y],
           ...
           print("--- %s seconds ---" % (time.time() - start_time2) + "Year" + str(y))
            ...
            GWP[y],TL[y],PL[y],GWT[y] = DPSpolicy(pump_capacity=pump_capacity, perennials_demandt_1=Perennials_water_end_t[y-1],
                               surface_watert=SOW["surface_water"].loc[swdi,y], maxland=land_max, perennials_landt_1 = Perennials_end_t[y-1],
                               C=C, R=R, RBFW1=newW1, RBFW2=newW2, RBFW3=newW3, RBFW4=newW4,
                               inputsr=(GW_depth_end_t[y-1]/max_depth,SOW["surface_water"].loc[swdi,y]/max_sw,Perennials_end_t[y-1]/land_max))
            
            scenario = scenario_run(year = y, sw_supply = SOW["surface_water"].loc[swdi,y], gwrest = GWP[y], peren_crops = Perennials_end_t[y-1],
                                   pump_tax=GWT[y],prices=SOW["Prices"].loc[s,y], sw_price=SOW["swprice"].loc[swdi,y], gwdepth=GW_depth_end_t[y-1],
                                   costs=costs, land_avail=TL[y], elec_price=SOW["Omegaelec"].loc[s,y], yields=np.array([1]*17),peren_restrict=PL[y],
                                   executable=executable_path, Land_t_1 = None) # This creates a dictionary that is the input for the model run
    ...
    print("Finished local insights")               
    results_dict[str(i)] = {"GWP":GWP.tolist(),"TL":TL.tolist(),"PL":PL.tolist(),"GWT":GWT.tolist(),"Pump_year":Pump_end_t.tolist(),
                            "Perennials_year":Perennials_end_t.tolist(),"GW_depth_year":GW_depth_end_t.tolist(),
                            "Net_revs": Net_revs_end_t.tolist(),"SW":SOW["surface_water"].loc[swdi,:].tolist(),
                            "Land_year":Total_land_end_t.tolist(), "Gross_revs": Gross_revs_end_t.tolist(),
                            "Perennial_water": Perennials_water_end_t.tolist()} #Results to visualize        

with open('robust_Borg_comp_wet.json','w') as fp:
